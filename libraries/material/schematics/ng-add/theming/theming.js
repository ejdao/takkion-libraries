...
const prebuiltThemePathSegment = '@takkion/material/prebuilt-themes';
...
async function insertCustomTheme(projectName, host, logger) {
  ...
  if (!stylesPath) {
    ...
    const customThemePath = (0, core_1.normalize)(
      (0, path_1.join)(project.sourceRoot, defaultCustomThemeFilename)
    );
...
function insertPrebuiltTheme(project, theme, logger) {
  const themePath = `@takkion/material/prebuilt-themes/${theme}.css`;
...
function validateDefaultTargetBuilder(project, targetName, logger) {
  const targets =
    targetName === 'test'
      ? (0, schematics_2.getProjectTestTargets)(project)
      : (0, schematics_2.getProjectBuildTargets)(project);
