 ...
 * @docs-private
 */
export function getTakIconNameNotFoundError(iconName) {
 ...
 * @docs-private
 */
export function getTakIconNoHttpProviderError() {
  return Error(
 ...
 * @docs-private
 */
export function getTakIconFailedToSanitizeUrlError(url) {
  return Error(
    `The URL provided to TakIconRegistry was not trusted as a resource URL ` +
 ...
 * @docs-private
 */
export function getTakIconFailedToSanitizeLiteralError(literal) {
  return Error(
    `The literal provided to TakIconRegistry was not trusted as safe HTML by ` +
...
export class TakIconRegistry {
  constructor(_httpClient, _sanitizer, document, _errorHandler) {
     ...
     */
    this._defaultFontSetClass = ['material-icons', 'tak-ligature-font'];
  ...
  addSvgIconLiteralInNamespace(namespace, iconName, literal, options) {
    ...
    if (!cleanLiteral) {
      throw getTakIconFailedToSanitizeLiteralError(literal);
    }
  ...
  getSvgIconFromUrl(safeUrl) {
    ...
    if (!url) {
      throw getTakIconFailedToSanitizeUrlError(safeUrl);
    }
  ...
  getNamedSvgIcon(name, namespace = '') {
    ...
    return observableThrow(getTakIconNameNotFoundError(key));
  }
  ngOnDestroy() {
    this._resolvers = [];
  ...
  _getSvgFromIconSetConfigs(name, iconSetConfigs) {
    ...
    return forkJoin(iconSetFetchRequests).pipe(
      map(() => {
        ...
        // TODO: add an ngDevMode check
        if (!foundIcon) {
          throw getTakIconNameNotFoundError(name);
        }
        return foundIcon;
      })
    );
  ...
  _fetchIcon(iconConfig) {
    ...
    if (!this._httpClient) {
      throw getTakIconNoHttpProviderError();
    }
    // TODO: add an ngDevMode check
    if (safeUrl == null) {
...
TakIconRegistry.ɵfac = i0.ɵɵngDeclareFactory({
  minVersion: '12.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakIconRegistry,
  deps: [
...
TakIconRegistry.ɵprov = i0.ɵɵngDeclareInjectable({
  minVersion: '12.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakIconRegistry,
...
export function ICON_REGISTRY_PROVIDER_FACTORY(
  ...
  return parentRegistry || new TakIconRegistry(httpClient, sanitizer, document, errorHandler);
}
/** @docs-private */
export const ICON_REGISTRY_PROVIDER = {
  // If there is already an TakIconRegistry available, use that. Otherwise, provide a new one.
  provide: TakIconRegistry,
  deps: [
    [new Optional(), new SkipSelf(), TakIconRegistry],
