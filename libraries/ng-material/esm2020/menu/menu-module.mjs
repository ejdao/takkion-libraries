/**
 * @license
 * Developed by Google LLC but not supported.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
import { OverlayModule } from '@takkion/ng-cdk/overlay';
import { CommonModule } from '@angular/common';
import { NgModule } from '@angular/core';
import { TakCommonModule, TakRippleModule } from '@takkion/ng-material/core';
import { CdkScrollableModule } from '@takkion/ng-cdk/scrolling';
import { TakMenu } from './menu';
import { TakMenuContent } from './menu-content';
import { TakMenuItem } from './menu-item';
import { TAK_MENU_SCROLL_STRATEGY_FACTORY_PROVIDER, TakMenuTrigger } from './menu-trigger';
import * as i0 from '@angular/core';
export class TakMenuModule {}
TakMenuModule.ɵfac = i0.ɵɵngDeclareFactory({
  minVersion: '12.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakMenuModule,
  deps: [],
  target: i0.ɵɵFactoryTarget.NgModule,
});
TakMenuModule.ɵmod = i0.ɵɵngDeclareNgModule({
  minVersion: '14.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakMenuModule,
  declarations: [TakMenu, TakMenuItem, TakMenuTrigger, TakMenuContent],
  imports: [CommonModule, TakCommonModule, TakRippleModule, OverlayModule],
  exports: [
    CdkScrollableModule,
    TakCommonModule,
    TakMenu,
    TakMenuItem,
    TakMenuTrigger,
    TakMenuContent,
  ],
});
TakMenuModule.ɵinj = i0.ɵɵngDeclareInjector({
  minVersion: '12.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakMenuModule,
  providers: [TAK_MENU_SCROLL_STRATEGY_FACTORY_PROVIDER],
  imports: [
    CommonModule,
    TakCommonModule,
    TakRippleModule,
    OverlayModule,
    CdkScrollableModule,
    TakCommonModule,
  ],
});
i0.ɵɵngDeclareClassMetadata({
  minVersion: '12.0.0',
  version: '14.2.0',
  ngImport: i0,
  type: TakMenuModule,
  decorators: [
    {
      type: NgModule,
      args: [
        {
          imports: [CommonModule, TakCommonModule, TakRippleModule, OverlayModule],
          exports: [
            CdkScrollableModule,
            TakCommonModule,
            TakMenu,
            TakMenuItem,
            TakMenuTrigger,
            TakMenuContent,
          ],
          declarations: [TakMenu, TakMenuItem, TakMenuTrigger, TakMenuContent],
          providers: [TAK_MENU_SCROLL_STRATEGY_FACTORY_PROVIDER],
        },
      ],
    },
  ],
});
//# sourceMappingURL=data:application/json;base64,
