...
import { TakOptionHarness } from './option-harness';
/** Harness for interacting with a `tak-optgroup` in tests. */
export class TakOptgroupHarness extends ComponentHarness {
  constructor() {
    super(...arguments);
    this._label = this.locatorFor('.tak-optgroup-label');
  }
  /**
   * Gets a `HarnessPredicate` that can be used to search for a `TakOptgroupHarness` that meets
   * certain criteria.
   * @param options Options for filtering which option instances are considered a match.
   * @return a `HarnessPredicate` configured with the given options.
   */
  static with(options = {}) {
    return new HarnessPredicate(TakOptgroupHarness, options).addOption(
      'labelText',
      options.labelText,
  ...
  async isDisabled() {
    return (await this.host()).hasClass('tak-optgroup-disabled');
  }
  /**
  ...
  async getOptions(filter = {}) {
    return this.locatorForAll(TakOptionHarness.with(filter))();
  }
}
/** Selector used to locate option group instances. */
TakOptgroupHarness.hostSelector = '.tak-optgroup';
