...
class HammerGesturesMigration extends schematics_1.DevkitMigration {
  ...
  visitTemplate(template) {
    if (!this._customEventsUsedInTemplate || !this._standardEventsUsedInTemplate) {
      const { standardEvents, customEvents } = (0,
      hammer_template_check_1.isHammerJsUsedInTemplate)(template.content);
  ...
  _setupHammerWithCustomEvents() {
    ...
    const newConfigPath = (0, core_1.join)(
      sourceRoot,
      this._getAvailableGestureConfigFileName(sourceRoot)
    );
    // Copy gesture config template into the CLI project.
    this.fileSystem.create(
      newConfigPath,
      (0, fs_1.readFileSync)(require.resolve(GESTURE_CONFIG_TEMPLATE_PATH), 'utf8')
    );
  ...
  _getAvailableGestureConfigFileName(sourceRoot) {
    if (
      !this.fileSystem.fileExists((0, core_1.join)(sourceRoot, `${GESTURE_CONFIG_FILE_NAME}.ts`))
    ) {
    ...
    let index = 1;
    while (
      this.fileSystem.fileExists((0, core_1.join)(sourceRoot, `${possibleName}-${index}.ts`))
    ) {
      index++;
    }
  ...
  _removeGestureConfigReference({ node, importData, isImport }) {
    ...
    const provideToken = objectLiteralExpr.properties.find(
      p => ts.isPropertyAssignment(p) && getPropertyNameText(p.name) === 'provide'
    );
  ...
  _removeHammerFromIndexFile() {
    ...
    indexFilePaths.forEach(filePath => {
      ...
      (0, find_hammer_script_tags_1.findHammerScriptImportElements)(htmlContent).forEach(el =>
        (0, remove_element_from_html_1.removeElementFromHtml)(el, recorder)
      );
    });
  }
  ...
  _setupNewGestureConfigInRootModule(gestureConfigPath) {
    ...
    const metadata = (0, schematics_1.getDecoratorMetadata)(
      sourceFile,
      'NgModule',
      '@angular/core'
    );
    ...
    if (
      ...
      (0, schematics_1.addSymbolToNgModuleMetadata)(
        sourceFile,
        sourceFile.fileName,
        'providers',
        symbolName,
...
function getModuleSpecifier(newPath, containingFile) {
  let result = (0, core_1.relative)((0, core_1.dirname)(containingFile), newPath)
    .replace(/\\/g, '/')
    .replace(/\.ts$/, '');
