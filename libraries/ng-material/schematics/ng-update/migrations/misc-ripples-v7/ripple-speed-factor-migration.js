...
class RippleSpeedFactorMigration extends schematics_1.Migration {
  ...
  visitTemplate(template) {
    ...
    while ((match = speedFactorNumberRegex.exec(template.content)) !== null) {
      const newEnterDuration = (0, ripple_speed_factor_1.convertSpeedFactorToDuration)(
        parseFloat(match[1])
      );
      this._replaceText(
        template.filePath,
    ...
    while ((match = speedFactorNotParseable.exec(template.content)) !== null) {
      const newDurationExpression = (0, ripple_speed_factor_1.createSpeedFactorConvertExpression)(
        match[1]
      );
      this._replaceText(
        template.filePath,
  ...
  _visitBinaryExpression(expression) {
    ...
    if (targetTypeName === 'TakRipple' && propertyName === 'speedFactor') {
      if (ts.isNumericLiteral(expression.right)) {
        ...
        const newExpression = (0, ripple_speed_factor_1.createSpeedFactorConvertExpression)(
