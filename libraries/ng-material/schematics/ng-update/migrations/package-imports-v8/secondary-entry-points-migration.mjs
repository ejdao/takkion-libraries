 ...
 */
const ANGULAR_TAKERIAL_FILEPATH_REGEX = new RegExp(
  `${module_specifiers_1.materialModuleSpecifier}/(.*?)/`
);
/**
...
class SecondaryEntryPointsMigration extends schematics_1.Migration {
  constructor() {
    ...
    this.enabled =
      this.targetVersion === schematics_1.TargetVersion.V8 ||
      this.targetVersion === schematics_1.TargetVersion.V9;
  }
  visitNode(declaration) {
    ...
    // import {TakCardModule, TakCardTitle} from '@takkion/ng-material/card';
      ...
      .map(([name, elements]) => {
        const newImport = ts.factory.createImportDeclaration(
          ...
          ts.factory.createStringLiteral(
            `${module_specifiers_1.materialModuleSpecifier}/${name}`,
            singleQuoteImport
          )
        );
...
function resolveModuleName(node, typeChecker) {
  ...
  const matches = sourceFile.match(ANGULAR_TAKERIAL_FILEPATH_REGEX);
