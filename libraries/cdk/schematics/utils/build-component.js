...
function buildDefaultPath(project) {
  ...
  const projectDirName =
    project.extensions.projectType === workspace_models_1.ProjectType.Application ? 'app' : 'lib';
...
function addDeclarationToNgModule(options) {
  return host => {
    ...
    const declarationChanges = (0, ast_utils_1.addDeclarationToModule)(
      source,
      modulePath,
      classifiedName,
      relativePath
    ...
    if (options.export) {
      ...
      const exportChanges = (0, ast_utils_1.addExportToModule)(
        source,
        modulePath,
...
function buildComponent(options, additionalFiles = {}) {
  return async (host, ctx) => {
    ...
    Object.keys(options)
      .filter(key => options[key] == null && defaultComponentOptions[key])
    ...
    for (let key in additionalFiles) {
      if (additionalFiles[key]) {
        const fileContent = (0, fs_1.readFileSync)(
          (0, path_1.join)(schematicFilesPath, additionalFiles[key]),
          'utf-8'
        );
    ...
    const templateSource = (0, schematics_1.apply)((0, schematics_1.url)(schematicFilesUrl), [
      options.skipTests
        ...
        : (0, schematics_1.noop)(),
      options.inlineStyle
        ? (0, schematics_1.filter)(path => !path.endsWith('.__style__.template'))
        : (0, schematics_1.noop)(),
