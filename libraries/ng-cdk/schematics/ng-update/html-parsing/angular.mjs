...
exports.findOutputsOnElementWithAttr =
  exports.findOutputsOnElementWithTag =
  exports.findInputsOnElementWithAttr =
  exports.findInputsOnElementWithTag =
    void 0;
...
function findInputsOnElementWithTag(html, inputName, tagNames) {
  return [
    // Inputs can be also used without brackets (e.g. `<tak-toolbar color="primary">`)
    ...
    ...(0, elements_1.findAttributeOnElementWithTag)(html, `[${inputName}]`, tagNames).map(
      offset => offset + 1
    ),
  ];
}
...
function findInputsOnElementWithAttr(html, inputName, attrs) {
  return [
    // Inputs can be also used without brackets (e.g. `<button tak-button color="primary">`)
    ...(0, elements_1.findAttributeOnElementWithAttrs)(html, inputName, attrs),
    // Add one column to the mapped offset because the first bracket for the @Input
    // is part of the attribute and therefore also part of the offset. We only want to return
    // the offset for the inner name of the bracketed input.
    ...(0, elements_1.findAttributeOnElementWithAttrs)(html, `[${inputName}]`, attrs).map(
      offset => offset + 1
    ),
  ];
}
...
function findOutputsOnElementWithTag(html, outputName, tagNames) {
  ...
  return (0, elements_1.findAttributeOnElementWithTag)(html, `(${outputName})`, tagNames).map(
    offset => offset + 1
  );
}
...
function findOutputsOnElementWithAttr(html, outputName, attrs) {
  ...
  // is part of the attribute and therefore also part of the offset. We only want to return
  // the offset for the inner name of the output.
  return (0, elements_1.findAttributeOnElementWithAttrs)(html, `(${outputName})`, attrs).map(
    offset => offset + 1
  );
}
